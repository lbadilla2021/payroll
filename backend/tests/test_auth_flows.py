...
def test_reset_token_expirado_y_reutilizado():
    ...
    token = PasswordResetToken(tenant_id=user.tenant_id, user_id=user.id, token_hash=hash_opaque_token(raw), expires_at=datetime.now(timezone.utc) - timedelta(minutes=1))
    db.add(token)
    db.commit()
    db.close()
...
def test_cambio_password_autenticado_y_auth_version():
    c = TestClient(app)
    login = _login(c)
...
def test_refresh_rotativo_logout_logout_all():
    ...
    assert c.post('/api/auth/logout', headers={'Authorization': f'Bearer {token}'}, json={'all_sessions': False}).status_code == 200
    assert c.get('/api/auth/me', headers={'Authorization': f'Bearer {token}'}).status_code == 401
...
def test_invalidacion_tras_reset_y_me_invalido(monkeypatch):
    monkeypatch.setattr('app.main.send_password_reset_email', lambda *args, **kwargs: None)
    c = TestClient(app)
    login = _login(c)
...
def test_cors_preflight_basico():
    c = TestClient(app)
